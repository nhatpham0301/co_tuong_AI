...
class GameManager {
  ...
  Future<bool> init() async {
    ...
    setting = GameSetting.getInstance();
  ...
  void on<T extends GameEvent>(void Function(GameEvent) listener) {
    ...
    if (type == null) {
      logger.warning('Kiểu sự kiện không khớp: ${T.runtimeType}');
      return;
    }
  ...
  void loadHistory(int index) {
    if (index >= manual.moveCount) {
      logger.info('Lỗi lịch sử: chỉ số vượt quá số nước đi');
      return;
    }
    if (index == _currentStep) {
      logger.info('Lịch sử không thay đổi');
      return;
    }
    _currentStep = index;
    ...
    logger.info('Tải lịch sử bước $_currentStep');
  }

  /// 切换驱动
  void switchDriver(int team, DriverType driverType) {
    logger.info('Chuyển driver người chơi $team sang ${driverType.name}');
  ...
  Future<void> next() async {
    ...
    logger.info('Có thể tiếp tục: $canNext');
    if (canNext) {
      switchPlayer();
    }
  }
  ...
  void addMove(PlayerAction action) {
    logger.info('Thêm nước đi: $action');
    ...
    if (!ChessManual.isPosMove(move)) {
      logger.info('Nước đi không hợp lệ: $move');
      return;
    }
  ...
  void setResult(String result, [String description = '']) {
    if (!ChessManual.results.contains(result)) {
      logger.info('Kết quả không hợp lệ: $result');
      return;
    }
    logger.info('Kết quả ván đấu: $result');
  ...
  bool checkResult(int hand, int curMove) {
    logger.info('Kiểm tra kết quả ván đấu');
    ...
    logger.info('Có chiếu tướng không: ${moveStep.isCheckMate}');
  ...
  void switchPlayer() {
    ...
    logger.info(
        'Chuyển lượt người chơi: $curHand - ${player.title} (${player.driverType.name})');

    logger.info('Người chơi hiện tại: ${player.title}');
    next();
    add(GameEngineEvent('clear'));
  }
  ...
  void requestHelp() async {
    if (engine.started) {
      logger.info('manager($hashCode) yêu cầu gợi ý nước đi');
      isStop = true;
      await engine.stop();
      engine.position(fenStr);
      await engine.go(depth: 10);
    } else {
      logger.info('Engine chưa được khởi động');
    }
  }
